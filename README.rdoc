...
=== Category Tags
  ...
  <r:category:current [depth="root"]> ... </r:category:current>
...
On products:each tag you can specify grabbing all products underneath the current
top-most category by setting include_subcategories to "true"

  <r:product:find [where=""]> ... </r:product:find>
  <r:products:each [where=""] [order="sequence ASC"] [include_subcategories="true"]> ... </r:products:each>
...
NOTE: This example assumes no subcategories 
  ...
  <r:categories:each order="title ASC">
    <div class="category <r:category:css_class />">
...
=== Current Category (for working with only the current category)

  <r:category:current>
    <h2>Products for <r:category:title /></h2>
    <div class="category <r:category:css_class />">
      <r:products:each order="price ASC">
        <div class="product">
          <h4><r:product:link /> (<r:product:price precision="0" />)</h4>
